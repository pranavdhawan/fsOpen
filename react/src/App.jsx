...
const Persons = ({ persons, setPersons }) => {
  const handleDelete = (person) => {
    personService.delete_(person.id)
      .then(response => {
        // Update the UI by removing the deleted person from the state
...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    if (persons.some((person) => person.name.toLowerCase() === newName.toLowerCase())) {
      ...
      // const persontobeupdated = persons.find((p) => { p.name.toLowerCase() === newName.toLowerCase() })

      // const newnumbertobeupdated = newNumber
